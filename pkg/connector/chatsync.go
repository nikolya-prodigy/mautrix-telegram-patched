...
func (tc *TelegramClient) handleDialogs(
	...
	for i, d := range dialogList {
		...
		case *tg.DialogCommunity:
			err = tc.syncCommunityDialog(ctx, dialog, createLimit < 0 || i < createLimit, chats)
		//case *tg.DialogFolder:
		default:
			continue
		}
...
func (tc *TelegramClient) syncNormalDialog(
	...
	if portal == nil || portal.MXID == "" {
		...
		approved, err := tc.ensurePortalApproved(ctx, portalKey, tc.portalApprovalInfoFromDialog(portalKey, dialog, users, chats), "dialog sync")
		if err != nil {
			return err
		} else if !approved {
			return nil
...
func (tc *TelegramClient) syncCommunityDialog(
	ctx context.Context, dialog *tg.DialogCommunity, allowCreate bool, chats map[int64]tg.ChatClass,
) error {
	log := zerolog.Ctx(ctx).With().
	...
	if !tc.main.Config.BridgeCommunities {
		log.Debug().Msg("Not syncing community because community bridging is disabled")
		return nil
	}

	community, ok := chats[dialog.CommunityID].(*tg.Community)
	if !ok {
		switch chat := chats[dialog.CommunityID].(type) {
		case *tg.CommunityForbidden:
			log.Debug().Msg("Not syncing portal because community is forbidden")
		default:
			log.Debug().
				Int64("community_id", dialog.CommunityID).
				Type("chat_type", chat).
	...
	if portal == nil || portal.MXID == "" {
		...
		approved, err := tc.ensurePortalApproved(
			ctx,
			portalKey,
			tc.portalApprovalInfoFromObject(portalKey, community),
			"community dialog sync",
		)
		if err != nil {
			return err
		} else if !approved {
	...
	chatInfo, _, err := tc.wrapChatInfo(portal.ID, community)
	if err != nil {
